...
class MPCPolicy(BasePolicy):
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        #       action sequence.
        obs_current = obs[None].repeat(N,0)
        for t in range(H):
            actions = candidate_action_sequences[:,t,:]
            obs_current = model.get_prediction(obs_current, actions,self.data_statistics)
            for i in range(N):
                sum_of_rewards[i] += self.env.get_reward(obs_current[i],actions[i])[0]
