...
class MBAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        indices = np.random.choice(num_data, (self.ensemble_size,num_data_per_env), replace=False)

        for i in range(self.ensemble_size):

            # select which datapoints to use for this model of the ensemble
            # you might find the num_data_per_env variable defined above useful
            observations = ob_no[indices[i]]
            actions = ac_na[indices[i]]
            next_observations = next_ob_no[indices[i]]

            # use datapoints to update one of the dyn_models
            model = self.dyn_models[np.random.choice(self.ensemble_size)]
